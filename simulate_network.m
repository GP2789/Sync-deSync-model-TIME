...
% defines Theta STDP multiplier, 1 for trough -1 for peak when theta_mod if
% false, no theta phase modulation
if par.theta_mod
   theta = 1-(osc.Hip/max(osc.Hip) + 1); 
else
   theta =  ones(size(osc.Hip,1),size(osc.Hip,2)); 
end
