...
if( ~isempty(spikes) )
    ...
    wc_pos(spikes, :, t) = (wc_pos(spikes, :, t) ...        % potentially contributing synapses
    ...
    if( par.STDP_th ) % FOR THRESHOLD INDUCED PLASTICITY
        ...
        %% Theta-phase-dependent learning only: constant = 0.02
        dW = weight_matrix_STDP(:, spikes) * theta * 0.02;      % modulation of theta
        p_w(:, spikes) = p_w(:, spikes) + dW;                        % add weight change
