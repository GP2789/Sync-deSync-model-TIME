...
if par{k}{i,j}{1}.stim_range
   b1 = (X1'*X1)\(X1'*theta_clouter(2,:)');
else
   % nonnegative linear least-squares for stim ranging between 0 and 1
   b1 = lsqnonneg(X1, theta_clouter(2,:)');
end
...
if par{k}{i,j}{1}.stim_range
   ...
   % nonnegative linear least-squares for stim ranging between 0 and 1
   b12 = lsqnonneg(X2, theta_clouter(2,:)');
end
...
legend('Data from Clouter et al., 2017',...
    ...
    ['Mean hippocampal weights predicted', newline, 'by the model with STDP only']...
    ,'FontSize',12,'EdgeColor','No','Color','none');
saveas(fig_clouter,['analyse_figures/' savefilename '/FullvsSTDP_fit_clouter.tiff']); close(fig_clouter);
...
legend('Data from Wang et al., 2018',...
    ...
    ['Mean hippocampal weights predicted', newline, 'by the model with STDP only']...
    ,'FontSize',12,'EdgeColor','No','Color','none');
saveas(fig_wang,['analyse_figures/' savefilename '/FullvsSTDP_fit_wang.tiff']); close(fig_wang);
