function [ vars ] = evaluate_network( variables, trials, filename, Hip_freq, input_freq, phase_offset )
    ...
    for i=1:v1_L % loop variable 1
        ...
        for j=1:v2_L % loop variable 2
            ...
            for t=1:trials % loop trials
                ...
                clearvars -global par; set_parameters; global par;
                if(isempty(vars.v1_vars)~=1); par.([vars.v1_parameter]) = vars.v1_vars{i}; end% change variable 1
                if(isempty(vars.v2_vars)~=1); par.([vars.v2_parameter]) = vars.v2_vars{j}; end% change variable 2
                if(isempty(vars.v3_vars)~=1); par.([vars.v3_parameter]) = vars.v3_vars{j}; end% change variable 3
                par.Hip_freq = Hip_freq(t);
                par.stim_F = input_freq(t);
                par.stim_G2_ph = phase_offset; 
%                 par.stim_mod = freq_mod;
%                 par.reset_ph = par.stim_G2_ph;
                recall_experiment(filename);
                vars.par{k}{i,j}{t} = par;
            end
        end
    end
end
