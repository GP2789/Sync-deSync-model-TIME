...
for g=1:length(nG)
    ...
    if(par.([nG{g} '_reset']) && strcmp(par.sim_type,'DL')==1) % isempty(strfind(par.sim_type,'idle'))) 
        ...
        if(strcmp(par.nG{g},'Hip')) % reset phase of EC mod as well
    ...
    plot(osc.Hip); hold on; plot(osc.EC); hold off;
...
%         else
%             mod_1 = (cos(2*pi*par.stim_F*(t+par.stim_G1_ph*(1000/par.stim_F)/360/1000)) + 1)/2;
%             mod_2 = (cos(2*pi*par.stim_F*(t+par.stim_G1_ph*(1000/par.stim_F)/360/1000)) + 1)/2;
%         end
    else
